...
import { grantStaffClinicAccess } from '@/lib/services/staffClinicAccess';
import { hasSupabaseConfig } from '@/lib/supabaseServer';
import { staffClinicAccessGrantSchema } from '@/lib/validators';
...
export const POST: APIRoute = async (context) => {
  ...
  if (!MANAGER_ROLES.has(role) && gate.user.role !== 'admin' && gate.user.role !== 'super_admin') {
    return fail('Solo administración puede asignar accesos.', 403);
  }

  try {
    const body = await context.request.json();
    const parsed = staffClinicAccessGrantSchema.safeParse(body);
    if (!parsed.success) return fail('Datos inválidos.', 422, parsed.error.flatten());

    const profile = await grantStaffClinicAccess({
      manager: gate.user,
      authUserId: parsed.data.authUserId,
      targetClinicId: parsed.data.clinicId,
      role: parsed.data.role,
      specialty: parsed.data.specialty,
      collegiateNumber: parsed.data.collegiateNumber
    });

    return ok({ profile }, { message: 'Acceso asignado a la clínica.' });
  } catch (error) {
    logError('clinic.users.assign-access', error);
    return fail(error instanceof Error ? error.message : 'No se pudo asignar el acceso.', 500);
  }
};
