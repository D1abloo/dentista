...
import { findSubscriptionDemo, getSubscriptionsDemo } from '@/lib/platform/subscriptionsDemo';
...
export const GET: APIRoute = async (context) => {
  ...
  const row = id ? findSubscriptionDemo(id) : getSubscriptionsDemo()[0];
  ...
  const lines = [
    ...
    `Asientos: ${row.seats_used} / ${row.seats_contracted}`,
    `MRR: ${row.mrr} EUR`,
    `Renovación: ${row.renewal_label}`,
    `Facturación: ${row.billing_status_label}`
  ];
  ...
  return new Response(new Blob([new Uint8Array(pdf)]), {
    headers: {
      ...
      'content-disposition': 'attachment; filename="informe-suscripciones.pdf"'
    }
  });
};
