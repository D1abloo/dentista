...
export const GET: APIRoute = async (context) => {
  ...
  if (!data.events.length) return fail('No se pudo exportar el informe.', 404);
  ...
  const lines = [
    ...
    `Último evento: ${data.kpis.last_event}`,
    `Retención: ${data.retention_days} días`
  ];

  return new Response(new Blob([new Uint8Array(buildMinimalPdf(lines))]), {
    headers: {
      'content-type': 'application/pdf',
      'content-disposition': 'attachment; filename="informe-auditoria.pdf"'
    }
  });
};
