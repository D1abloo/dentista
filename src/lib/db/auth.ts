import { getPgPool } from '@/lib/db/pgPool'
...
export async function localSignInWithPassword(email: string, password: string) {
  try {
    ...
    await pool.query(`update auth.users set last_sign_in_at = now(), updated_at = now() where id = $1`, [row.id])
    const user = mapUser(row)
    return {
      data: { user, session: { access_token: 'pg', user } },
      error: null
    }
  } catch (err) {
...
/** @deprecated */
export const isLocalPostgresMode = () => true
