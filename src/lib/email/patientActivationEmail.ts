...
export async function sendPatientActivationEmail(input: {
  ...
  const text = [
    `Hola ${input.fullName},`,
    '',
    `Gracias por registrarte en AgendaClinic para gestionar tus citas en ${input.clinicName}.`,
    '',
  ...
  const html = `
    <div style="font-family:system-ui,sans-serif;max-width:520px;color:#0f172a">
      ...
      <p>Gracias por registrarte en <strong>AgendaClinic</strong> para ${input.clinicName}.</p>
  ...
  return sendMail({
    to: input.email,
    subject: 'Activa tu cuenta de paciente · AgendaClinic',
    text,
    html,
...
export async function sendPatientRegistrationPendingEmail(input: {
  ...
  return sendMail({
    to: input.email,
    subject: 'Registro recibido — activa tu cuenta · AgendaClinic',
    text,
    html,
